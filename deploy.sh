...
EFS="172.31.33.253:/opt/apps"

cat scripts/aliases.tmpl | sed "s#CERT#${CERT}#" > scripts/aliases.sh
cat scripts/install-rsw.sh.tmpl | sed "s#S3_BUCKETNAME#${S3_BUCKETNAME}#g" | sed "s#EFS#${EFS}#" > scripts/install-rsw.sh
cat scripts/install-compute.sh.tmpl | sed "s#S3_BUCKETNAME#${S3_BUCKETNAME}#g" | sed "s#EFS#${EFS}#" > scripts/install-compute.sh
